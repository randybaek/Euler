...
/**
 * Created by Randy Baek on 10/1/15.
 * re-use Prob008
 */
object Prob011 extends EulerApp {
  ...
  // matrix(row)(column)
  // TODO How to map each elements in Array of Array?
  ...
  def diagonalMaximum(n: Int): Int = {
    ...
    val startsFromUpperLeft = (3 to 16 map ((0, _))) ++ (1 to 16 zip Stream.continually(16))
    // append following elements until it satisfy the restrictions.
    val dd = startsFromUpperRight.map(
      start => {
        (0 to 19).takeWhile(a => (a + start._1 < 20) & (a + start._2 < 20)).map(add => matrix(start._1 + add)(start._2 + add)).toArray
      }
    )

    val ll = startsFromUpperLeft.map(
      start => {
  ...
  def findGreatestProductOfNAdjacentDigits(input:Array[String], n: Int) : Int =
    input.map(Integer.parseInt(_)).sliding(n).map(_.product).max
